...
1.  **Generate Index File:** Create `conductor/index.md`:
    ...
    ```
2.  **Persist Conductor Context (CRITICAL — this is what makes the resolution protocol available at runtime):**
    -   Claude Code does **NOT** auto-load this plugin's own `CLAUDE.md`. The Universal File Resolution Protocol must therefore be written where Claude Code *does* load context: the project-root `CLAUDE.md` (auto-loaded from cwd + parents) and an in-project copy at `conductor/CONTEXT.md`.
    a.  **Write `conductor/CONTEXT.md`** containing exactly the canonical protocol text below.
    b.  **Inject the same text into the project-root `CLAUDE.md`** (create the file if it does not exist), wrapped between the literal markers `<!-- BEGIN CONDUCTOR CONTEXT -->` and `<!-- END CONDUCTOR CONTEXT -->`. **If those markers already exist, REPLACE the content between them** (idempotent — never append a second block).
    c.  Stage both files so they are included in the setup commit.

    **Canonical protocol text (write verbatim to BOTH locations):**
    ...
    **PROTOCOL: How to locate files.**
    ...
    5.  **Verify:** You MUST verify the resolved file actually exists on the disk using the Read tool.
    ...
    **Skills Paths:**
    - **Project-level skills**: `conductor/skills/`
    - **Global skills**: `~/.claude/conductor-skills/`

    **Commit Message Conventions:**
    - Setup: `conductor(setup): Add conductor setup files`
    - New track: `chore(conductor): Add new track '<description>'`
    - Task complete: `conductor(plan): Mark task '<name>' as complete`
    - Track complete: `chore(conductor): Mark track '<description>' as complete`
    - Doc sync: `docs(conductor): Synchronize docs for track '<description>'`
    - Archive: `chore(conductor): Archive track '<description>'`
    - Review fixes: `fix(conductor): Apply review suggestions for track '<name>'`
    ```

3.  **Summarize Actions:** Present a summary of all setup actions taken.
4.  **Transition:** Announce that setup is complete and proceed to define the first track.
